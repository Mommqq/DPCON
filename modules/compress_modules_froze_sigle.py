...
class Compressor(nn.Module):
    ...
    def build_network(self):
        ...
        self.dec_Px = nn.ModuleList([])
        self.dec_xy = nn.ModuleList([])       
        self.dec_y = nn.ModuleList([])
        self.dec_Py = nn.ModuleList([])
    ...
    def decode_xy(self, x, y, cond=None):
        output = []
        output_y = []
        x = torch.cat((x,y),1)
        # y = torch.cat((y,w),1)
    ...
    def decode_x(self, x, cond=None):
        output = []

        for i, (resnet_x, vbrscaler_x, up_x) in enumerate(self.dec_x):

            # if i>=2 and i<=3:
                # print(output_y[4-i].shape)

            x = resnet_x(x)
            x = up_x(x)
            output.append(x)
            # print(output_y[i].shape)
            # print(x.shape)
    ...
    def bpp(self, shape, state4bpp_x, state4bpp_y):
        ...
        # hyper_rate_w = -self.prior_w.likelihood(hyper_latent_w).log2()
        # cond_rate_w = -latent_distribution_w.likelihood(latent_w).log2()
        # bpp_w = (hyper_rate_w.sum(dim=(1, 2, 3)) + cond_rate_w.sum(dim=(1, 2, 3))) / (H * W)
        return bpp_x,bpp_y
    ...
    def forward(self, input_x,input_y, cond=None):
        ...
        output,output_y = self.decode_xy(q_latent, latent_y, cond)
        # output_cond, output_y_cond = self.decode_xy_cond(q_latent,latent_y,cond)
        # output_cond = self.decode_x(q_latent,output[::-1], cond)
        output_cond = self.decode_x(q_latent, cond)
        # print(output_cond[0].shape,output_cond[1].shape,output_cond[2].shape,output_cond[3].shape)
        # print(y.shape)
        return {
            "output": output,
